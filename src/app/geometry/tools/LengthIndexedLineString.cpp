...
ign::geometry::Point	LengthIndexedLineString::locateAlong( double const& s ) const
{
	...
	ign::math::Vector3T< double > vA = _lineString[ numSeg ].toVec3d();
	ign::math::Vector3T< double > vB = _lineString[ numSeg + 1 ].toVec3d();
    if (!_is3d) { vA[2]=0; vB[2]=0; } // on ajoute le z a zero
    
	ign::math::Vector3T< double > AB = (vB - vA);
	double n = AB.norm();
...
void	LengthIndexedLineString::_computeIndex()
{
	...
	for ( size_t i = 0; i < _lineString.numPoints() - 1 ; i++ ){
		...
		if (_is3d) L += std::sqrt(dx*dx+dy*dy+dz*dz);
        else L += std::sqrt(dx*dx+dy*dy);
#endif
	}
	_index.back() = L;
}
